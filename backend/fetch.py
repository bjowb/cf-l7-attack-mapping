...
async def fetch_events(limit : int = 50) -> Dict[str,Any]:
    ...
    if api fails, get randomized results.
    we will check the code if it run successfully or not.
    i hope we can fetch api successfully today.
    """
    ...
    #if arcs is empty i.e. api fetch failed, we generate random attacks
    if not arcs :
